import torch
import torch.nn.functional as F
from tqdm import tqdm

from utils.dice_score import multiclass_dice_coeff, dice_coeff


@torch.inference_mode()
def evaluate(net, dataloader, device, amp):
    net.eval()
    num_val_batches = len(dataloader)
    dice_score = 0
    total_correct = 0  # 用于存储正确分类的像素数量
    total_pixels = 0  # 用于存储总像素数量

    # iterate over the validation set
    with torch.autocast(device.type if device.type != "mps" else "cpu", enabled=amp):
        for batch in tqdm(
            dataloader,
            total=num_val_batches,
            desc="Validation round",
            unit="batch",
            leave=False,
        ):
            image, mask_true = batch["image"], batch["mask"]

            # move images and labels to correct device and type
            image = image.to(
                device=device, dtype=torch.float32, memory_format=torch.channels_last
            )
            mask_true = mask_true.to(device=device, dtype=torch.long)

            # predict the mask
            mask_pred = net(image)

            if net.n_classes == 1:
                assert (
                    mask_true.min() >= 0 and mask_true.max() <= 1
                ), "True mask indices should be in [0, 1]"
                mask_pred = (F.sigmoid(mask_pred) > 0.5).float()
                # compute the Dice score
                dice_score += dice_coeff(mask_pred, mask_true, reduce_batch_first=False)

                # 计算准确率，忽略背景
                total_correct += (mask_pred == mask_true).sum().item()
                total_pixels += mask_true.numel()
            else:
                assert (
                    mask_true.min() >= 0 and mask_true.max() < net.n_classes
                ), "True mask indices should be in [0, n_classes]"
                # convert to one-hot format
                mask_true = (
                    F.one_hot(mask_true, net.n_classes).permute(0, 3, 1, 2).float()
                )
                mask_pred = (
                    F.one_hot(mask_pred.argmax(dim=1), net.n_classes)
                    .permute(0, 3, 1, 2)
                    .float()
                )
                # compute the Dice score, ignoring background
                dice_score += multiclass_dice_coeff(
                    mask_pred[:, 1:], mask_true[:, 1:], reduce_batch_first=False
                )

                # 计算准确率，忽略背景
                mask_pred_labels = mask_pred.argmax(dim=1)
                mask_true_labels = mask_true.argmax(dim=1)

                # 只计算非背景像素
                valid_pixels = mask_true_labels != 0
                total_correct += (mask_pred_labels[valid_pixels] == mask_true_labels[valid_pixels]).sum().item()
                total_pixels += valid_pixels.sum().item()  # 只计算有效像素的数量

    net.train()
    accuracy = total_correct / total_pixels if total_pixels > 0 else 0
    return dice_score / max(num_val_batches, 1), accuracy