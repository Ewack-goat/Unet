...
dir_img = Path("./data/imgs_Light/")
dir_mask = Path("./data/mask2/")
...
def train_model(
    ...
    for epoch in range(1, epochs + 1):
        ...
        # Evaluation round
        model.eval()  # 切换到评估模式
        val_score, accuracy = evaluate(model, val_loader, device, amp)  # 验证模型
        logging.info(
            "Validation Dice score: {:.4f}Accuracy:{:.4f} ".format(val_score, accuracy)
        )
